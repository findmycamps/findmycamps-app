        ...
        return (
          <React.Fragment key={step}>
            <div className="flex flex-col items-center text-center">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 font-bold ${isActive ? "bg-[#00524C] text-[#FFFBEB]" : isCompleted ? "bg-[#ACDB95] text-[#00524C]" : "bg-[#ACDB95]/30 text-[#00524C]/70"}`}
              >
                {isCompleted ? "✓" : stepNumber}
              </div>
              <p
                className={`mt-2 text-xs font-semibold ${isActive || isCompleted ? "text-[#00524C]" : "text-[#00524C]/70"}`}
              >
                {step}
              </p>
            </div>
            {index < steps.length - 1 && (
              <div
                className={`flex-auto h-1 mx-2 transition-all duration-300 rounded-full ${isCompleted ? "bg-[#ACDB95]" : "bg-[#ACDB95]/30"}`}
              ></div>
            )}
          </React.Fragment>
        );
  ...
  return (
    <div className="flex items-center gap-2">
      <Button
        type="button"
        variant="outline"
        className="border-[#ACDB95] text-[#00524C] hover:bg-[#ACDB95]/50"
        size="icon"
      ...
      <Input
        className="text-center w-16 bg-white border-[#ACDB95] text-[#00524C] focus-visible:ring-[#00524C]"
        type="text"
        value={value}
        readOnly
      />
...
export default function AuthPage() {
  ...
  const handleNumKidsChange = (count: number) => {
    ...
    if (count > currentCount) {
      const newKids = Array(count - currentCount)
        .fill(0)
        .map(() => ({ name: "", age: "", interests: [] }));
  ...
  const handleNextStep = (e: React.FormEvent) => {
    ...
    if (!lookingFor.parent && !lookingFor.self)
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    if (!city || !province) {
      setLoading(false);
      return setError("Please provide your location information.");
    }
    ...
    try {
      ...
      const userProfile: UserProfile = {
        ...
        children: lookingFor.parent
          ? children.filter((c) => c.name && c.age)
          : [],
  ...
  const commonInputStyles =
    "bg-white border-[#ACDB95] text-[#00524C] focus:border-[#00524C] focus:ring-[#00524C]";
    ...
    <div className="min-h-screen bg-[#FFFBEB] text-[#00524C]">
      ...
      <main className="flex items-center justify-center mt-8 md:mt-16 p-4">
        <div className="w-full max-w-2xl p-8 space-y-6 bg-[#FFFBEB] border border-[#ACDB95] rounded-lg shadow-lg">
          ...
          {error && (
            <div className="p-3 text-center text-sm text-red-700 bg-red-100 border border-red-300 rounded-md animate-shake">
              {error}
            </div>
          )}
          ...
          {isLoginView ? (
            ...
            <div className="animate-fade-in">
              <div className="text-center mb-6">
                <h1 className="text-3xl font-bold text-[#00524C]">
                  Welcome Back!
                </h1>
                <p className="text-[#00524C]/80">
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <Button
                  type="submit"
                  className="w-full bg-[#00524C] text-[#FFFBEB] hover:bg-[#00524C]/90 font-semibold"
                  disabled={loading}
                >
            ...
            <div className="animate-fade-in">
              <form onSubmit={handleNextStep} className="space-y-6">
                ...
                <div>
                  <label htmlFor="email-signup" className="font-medium">
                    Email
                  </label>
                  <Input
                    id="email-signup"
                ...
                <div>
                  <label htmlFor="password-signup" className="font-medium">
                    Password
                  </label>
                  <Input
                    id="password-signup"
                ...
                <div>
                  <label className="font-medium text-center block mb-3">
                    What are you looking for?
                  </label>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div
                      onClick={() => handleLookingForChange("parent")}
                      className={`p-4 border-2 rounded-lg cursor-pointer transition-all duration-200 ${lookingFor.parent ? "border-[#00524C] bg-[#ACDB95]/30" : "border-[#ACDB95] hover:border-[#00524C]"}`}
                    >
                      <Users className="mx-auto h-8 w-8 mb-2 text-[#00524C]" />
                      <p className="text-center font-semibold">
                        For my child(ren)
                      </p>
                      <p className="text-center text-xs text-[#00524C]/80">
                        I'm a parent or guardian.
                      </p>
                    </div>
                    <div
                      onClick={() => handleLookingForChange("self")}
                      className={`p-4 border-2 rounded-lg cursor-pointer transition-all duration-200 ${lookingFor.self ? "border-[#00524C] bg-[#ACDB95]/30" : "border-[#ACDB95] hover:border-[#00524C]"}`}
                    >
                      <UserIcon className="mx-auto h-8 w-8 mb-2 text-[#00524C]" />
                      <p className="text-center font-semibold">For myself</p>
                      <p className="text-center text-xs text-[#00524C]/80">
                        I'm looking for camps.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="animate-fade-in">
              <form onSubmit={handleFinalSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Select onValueChange={setProvince} value={province}>
                      ...
                      <SelectContent className="bg-white text-[#00524C]">
                  ...
                  {lookingFor.self && (
                    <div className="space-y-4 p-4 border border-[#ACDB95] rounded-lg bg-[#FFFBEB] animate-fade-in">
                      <h3 className="font-semibold flex items-center gap-2">
                        <UserIcon className="h-5 w-5 text-[#00524C]" /> Your
                        Personal Interests
                      </h3>
                      <TagSelector
                        selectedTags={personalInterests}
                        onTagToggle={handlePersonalInterestToggle}
                      />
                    </div>
                  )}
                  {lookingFor.parent && (
                    <div className="space-y-4 p-4 border border-[#ACDB95] rounded-lg bg-[#FFFBEB] animate-fade-in">
                      <h3 className="font-semibold flex items-center gap-2">
                        <Users className="h-5 w-5 text-[#00524C]" /> Your
                        Child(ren)'s Details
                      </h3>
                      <div>
                        <label
                      ...
                      <div className="space-y-3 max-h-60 overflow-y-auto pr-2">
                        {children.map((child, index) => (
                          <div
                            key={index}
                            className="p-3 border border-[#ACDB95]/50 rounded-md space-y-3 bg-[#FFFBEB] animate-fade-in-slide"
                          >
                            ...
                            <div>
                              ...
                              <TagSelector
                                selectedTags={child.interests}
                                onTagToggle={(tag) =>
                                  handleChildInterestToggle(index, tag)
                                }
                              />
                            </div>
                          </div>
                ...
                <div className="flex justify-between items-center">
                  <Button
                    variant="ghost"
                    className="text-[#00524C] hover:bg-[#ACDB95]/30"
                  ...
                  <Button
                    type="submit"
                    className="bg-[#00524C] text-[#FFFBEB] hover:bg-[#00524C]/90 font-semibold"
                    disabled={loading}
                  >
            ...
            >
              {isLoginView ? (
                <span>
                  ...
                  <span className="underline transition-opacity hover:opacity-80">
                    Sign Up
                  </span>
                </span>
              ) : (
