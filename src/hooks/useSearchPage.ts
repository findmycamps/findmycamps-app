...
export const useSearchPage = () => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log(
        `🏙️ Location search for "${searchCriteria.location}": Found ${filtered.length} camps`,
      );
    } else if (
      searchCriteria.location !== "ALL" &&
      searchCriteria.locationType === "all"
    ) {
      filtered = filtered.filter(
  ...
  const handleCategoryChange = (category: string, checked: boolean) => {
    if (checked) {
      ...
      if (searchCriteria.selectedCategories?.includes(category)) {
        const newCategories = searchCriteria.selectedCategories.filter(
          (cat) => cat !== category,
        );
