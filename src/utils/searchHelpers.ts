...
export const extractAgeRange = (ageString: string): { min: number; max: number } | null => {
  const cleaned = ageString.toLowerCase().replace(/years?/g, '').trim();
...
export const generateResultsTitle = (searchCriteria: any): string => {
  ...
  if (searchCriteria.selectedCategories && searchCriteria.selectedCategories.length > 0) {
    const categoryText = searchCriteria.selectedCategories.length === 1 
    ...
    if (searchCriteria.locationType === 'nearby') {
      title = searchCriteria.keyword && searchCriteria.keyword !== 'Nearby'
        ? `"${searchCriteria.keyword}" camps near you` 
        : 'Camps near your location';
    } else if (searchCriteria.locationType === 'specific') {
      title = searchCriteria.keyword && searchCriteria.keyword !== searchCriteria.location
  ...
  if (searchCriteria.dateRange?.from) {
    if (searchCriteria.dateRange.to) {
      const fromStr = searchCriteria.dateRange.from.toLocaleDateString('en-US', { 
        month: 'short', 
        day: 'numeric' 
      });
      ...
      const dateStr = searchCriteria.dateRange.from.toLocaleDateString('en-US', { 
        month: 'long', 
        day: 'numeric',
        year: 'numeric' 
      });
